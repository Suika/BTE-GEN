...
$sql="SELECT DL_ID, DL_Link, Book_Vol, Book_Name FROM LNDL, Book, Series WHERE LNDL.User_ID = $UID AND Series.Ser_ID = $SID AND LNDL.Book_Book_ID = Book.Book_ID AND Book.Series_Ser_ID = Series.Ser_ID AND LNDL.User_ID <> 1 AND LNDL.Closed = 0";
...
echo "<div><p><a href=\"" . urldecode($zeile['DL_Link']) . "\" title=\"" . urldecode($zeile['DL_Link']) . "\">" . $title . "</a><input type=\"button\" value=\"Remove\" onClick=\"closeLink($zeile[DL_ID]);\" text-align=\"right\"></p></div>";
 } 
mysqli_close($db); 
?>
<?php
