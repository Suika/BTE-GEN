...
$sql="SELECT ID FROM User, User_has_Groups WHERE LOWER(`Name`) = '" . mysqli_real_escape_string($conid, $_SESSION['benutzername']) . "' AND User_has_Groups.User_ID = User.ID AND User.Closed = 0";
$result = mysqli_query($db, $sql);
$zeile=mysqli_fetch_array($result);

$sql="UPDATE LNDL SET Closed = 1 WHERE DL_ID = $id AND User_ID = $zeile[ID] LIMIT 1";
